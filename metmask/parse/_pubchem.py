...
class pubchemParser:
    ...
    def queryPubchem (self, un, table) :
        ...
        if un.hasTable(table) :
            for ide in un.getIdentifiers(table)  :
                qres = self.wsdlserver.run_eSearch(tool=self.tool, email=self.mail, term="\"" + ide + "\"", field=self.fields[table], retmax=self.MAX_RET, db=self.db, usehistory='y')
    ...
    def getPubchemMasks (self, un) :
        ...
        if not hit :
            return(res)
        try :
            summaryUrl = self.summary + '&WebEnv=' + self.queryResult['WebEnv'] + "&query_key=" + self.queryResult['QueryKey']
